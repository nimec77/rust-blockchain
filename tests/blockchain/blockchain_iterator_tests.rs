...
fn test_iterator_with_different_hash_formats() {
    ...
    for (i, hash) in hash_formats.iter().enumerate() {
        let mut block = create_test_block("prev".to_string(), i);
...
fn test_iterator_database_operations() {
    ...
    // Verify all block properties are maintained
    assert_eq!(retrieved_block.get_hash(), "test_current");
    assert_eq!(retrieved_block.get_pre_block_hash(), "test_prev");
    assert_eq!(retrieved_block.get_height(), 5);
    assert_eq!(retrieved_block.get_transactions().len(), 1);
}
...
fn test_iterator_with_single_block_chain() {
    ...
    // First call should return the genesis block
    let result1 = iterator.next();
    assert!(result1.is_some());
    let block1 = result1.unwrap();
    assert_eq!(block1.get_hash(), "genesis_only");
    assert_eq!(block1.get_height(), 0);
    assert_eq!(iterator.current_hash, "");
